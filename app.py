...
def make_args_parser():
    ...
    parser.add_argument('--log', default='info',
                        ...
                        help='Log level')
    parser.add_argument('--modbus-listen-addres', dest='address', default='0.0.0.0',
                        help='Address for the Modbus slave (server) to listen on.')
    parser.add_argument('--modbus-listen-port', dest='port', default='502',
...
def make_openweathermap_request(args):
    lat, lon = get_lat_lon()
    resp=requests.get(OPENWEATHERMAP_API, params=dict(lat=lat, lon=lon, appid=args.api_key)).json()
